...
`coupling_strength` means lambda. In particular, the paper's legacy stored
parameter alpha equals lambda squared. Do not put alpha into the bath and
also supply its square root as coupling strength.
...
inside the spectral density. A user-supplied correlation takes precedence;
no spectral normalization is guessed.
...
The retained internal TCL6 engine returns an interaction-picture coefficient;
the public interface transforms it to the same picture as the lower orders.
...
The middle RK4 stages use the calculated midpoint generator. They do not use
averaged endpoint generators. All returned density matrices are transformed
back to the input laboratory basis.
...
Evolution never applies trace renormalization, Hermitization, clipping of
negative eigenvalues, or a dynamical-map inverse. Trace and Hermiticity checks
are numerical diagnostics. Positivity is not guaranteed by a finite TCL
truncation. The original paper curves are finite-grid results; reproducing a
curve does not establish continuum convergence.
